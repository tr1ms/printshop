...
passport.use(new LocalStrategy(
	async (username, password, done) => {
		const isAdmin = username.toLowerCase() == "admin" && password == adminPassword
		if (isAdmin) return done(null, { username: 'admin', email: '' })
		const user = await User.get(username)
		const isUser = user && await bcrypt.compare(password, user.password)
		if (isUser) return done(null, { username: user.username, email: user.email })
...
router.post("/login", passport.authenticate('local', { session: false }), (req, res, next) => {
	const token = jwt.sign(req.user, jwtSecret, { algorithm: 'HS256', expiresIn: process.env.JWT_EXPIRES || '30d' })
